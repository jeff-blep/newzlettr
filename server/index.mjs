...
// Absolute API base for server-side fetches (used while rendering emails on the server)
const PORT_ENV = process.env.PORT || 3001;
const SERVER_BASE = PUBLIC_ORIGIN || `http://127.0.0.1:${PORT_ENV}`;
const apiUrl = (p) => `${SERVER_BASE}${p}`;

/* ------------------------------- config I/O -------------------------------- */
const CONFIG_PATH = path.join(__dirname, "config.json");
const CLOUDINARY_MAP_PATH = path.join(__dirname, "cloudinary_urls.json");
const DEFAULT_CONFIG = {
  ...
  imageHost: "embedded", // or "cloudinary"
  cloudinary: { cloudName: "", apiKey: "", apiSecret: "", folder: "newzlettr" },
};

// Cloudinary static URL map (generated by server/scripts/upload-static-images.mjs)
function loadCloudinaryMap() {
  try {
    if (fs.existsSync(CLOUDINARY_MAP_PATH)) {
      const txt = fs.readFileSync(CLOUDINARY_MAP_PATH, "utf8");
...
let CLOUDINARY_MAP = loadCloudinaryMap();

// Simple deep merge for plain objects/arrays
...
function loadConfig() {
  ...
  // Merge order: DEFAULT < base(primary)
  const merged = deepMerge(DEFAULT_CONFIG, base);

  // Normalize shapes & defaults
  const out = {
    ...DEFAULT_CONFIG,
    ...merged,
    lastTest: { ...DEFAULT_CONFIG.lastTest, ...(merged.lastTest || {}) },
    lookbackDays: typeof merged.lookbackDays === "number" ? merged.lookbackDays : 7,
    ownerRecommendation: merged.ownerRecommendation || {},
  };
...
function saveConfig(cfg) {
  ...
  try { fs.writeFileSync(CONFIG_PATH, pretty, "utf8"); } catch (e) { console.error("[config] write primary failed:", e?.message || e); }
}

function sanitizeConfigForClient(cfg) {
  const out = {
    // SMTP (frontend expects these names)
    fromAddress: cfg.fromAddress || cfg?.smtp?.fromAddress || "",
    smtpEmailLogin: cfg.smtpEmailLogin || cfg?.smtp?.user || cfg.smtpUser || "",
    smtpServer: cfg.smtpServer || cfg?.smtp?.host || cfg.smtpHost || "",
    smtpPort: typeof cfg.smtpPort === "number" ? cfg.smtpPort : (cfg?.smtp?.port ?? 587),
    smtpEncryption: cfg.smtpEncryption || (cfg?.smtp?.secure ? "TLS/SSL" : "STARTTLS"),

    // Plex/Tautulli (flat keys preserved for UI)
    plexUrl: cfg.plexUrl || cfg?.plex?.url || "",
    plexToken: cfg.plexToken || cfg?.plex?.token || "",
    tautulliUrl: cfg.tautulliUrl || cfg?.tautulli?.url || "",
    tautulliApiKey: cfg.tautulliApiKey || cfg?.tautulli?.apiKey || "",

    // UI extras
    lookbackDays: typeof cfg.lookbackDays === "number" ? cfg.lookbackDays : 7,
    ownerRecommendation: cfg.ownerRecommendation || {},

    // Image hosting
    imageHost: cfg.imageHost || "embedded",
    cloudinary: cfg.cloudinary
      ? {
          cloudName: cfg.cloudinary.cloudName || cfg.cloudinary.cloud_name || "",
          apiKey: cfg.cloudinary.apiKey || cfg.cloudinary.api_key || "",
          apiSecret: cfg.cloudinary.apiSecret ? "******" : "", // mask for client
          folder: cfg.cloudinary.folder || cfg.cloudinary.path || "newzlettr",
        }
      : {
          cloudName: "",
          apiKey: "",
...
try {
  saveConfig(CONFIG);
  console.log("[config] merged + synced to:", CONFIG_PATH);
} catch {}
...
app.all('/api/tautulli/*', (req, res, next) => {
  if (req.headers['x-internal-render'] === '1') return next();
  try {
...
// Reload Cloudinary map (for development convenience)

app.post("/api/_reload_cloudinary_map", (_req, res) => {
  CLOUDINARY_MAP = loadCloudinaryMap();
  res.json({ ok: true, keys: Object.keys(CLOUDINARY_MAP || {}) });
});

// Clear Cloudinary folder (delete resources by prefix and reset local map)
...
app.post("/api/cloudinary/clear", async (req, res) => {
  try {
    ...
    const folder = String((body.folder || cloud.folder || "newzlettr")).replace(/^\/+/, "");
    const prefix = folder.endsWith("/") ? folder : `${folder}/`;

    // Safety: require Cloudinary mode
    if (String(cfg.imageHost || "").toLowerCase() !== "cloudinary") {
      return res.status(400).json({ ok: false, error: "Image host is not set to Cloudinary." });
    }

    // Lazy import to avoid duplicate top-level import
    const { default: cloudinary } = await import("./cloudinary.mjs");

    // Ensure Cloudinary SDK is configured (prefer CONFIG, fallback to ENV)
    const cloudName = String(cfg?.cloudinary?.cloudName || process.env.CLOUDINARY_CLOUD_NAME || "");
    const apiKey    = String(cfg?.cloudinary?.apiKey    || process.env.CLOUDINARY_API_KEY    || "");
    const apiSecret = String(cfg?.cloudinary?.apiSecret || process.env.CLOUDINARY_API_SECRET || "");
    if (!cloudName || !apiKey || !apiSecret) {
      return res.status(400).json({ ok: false, error: "Missing Cloudinary credentials (cloudName/apiKey/apiSecret)" });
    }
    if (typeof cloudinary.config === "function") {
      cloudinary.config({ cloud_name: cloudName, api_key: apiKey, api_secret: apiSecret });
    }

    // Delete originals by prefix (may need to loop if there are many)
    let next_cursor;
    do {
      ...
      if (ids.length) {
        await cloudinary.api.delete_resources(ids, { resource_type: "image", invalidate: true });
        // Also delete derived assets (transformations) explicitly
        try { await cloudinary.api.delete_all_resources({ public_ids: ids, resource_type: "image", keep_original: true }); } catch {}
      }
      next_cursor = list.next_cursor;
    } while (next_cursor);

    // Try to delete the folder if empty
    try { await cloudinary.api.delete_folder(prefix.replace(/\/$/, "")); } catch {}

    // Reset local static URL map so future sends/uploads repopulate as needed
    try {
      fs.writeFileSync(CLOUDINARY_MAP_PATH, JSON.stringify({}, null, 2), "utf8");
      CLOUDINARY_MAP = {};
    } catch (e) {
      console.warn("[cloudinary] failed to reset map:", e?.message || e);
    }
...
// List Cloudinary resources under a folder prefix (debug)
if (process.env.NODE_ENV !== "production") app.get("/api/cloudinary/list", async (req, res) => {
  try {
    const cfg = CONFIG || {};
    if (String(cfg.imageHost || "").toLowerCase() !== "cloudinary") {
      return res.status(400).json({ ok: false, error: "Image host is not set to Cloudinary." });
    }
    const qPrefix = (req.query.prefix || req.query.folder || cfg?.cloudinary?.folder || "newzlettr");
    const prefix = String(qPrefix).replace(/^\/+/, "");
    ...
    do {
      ...
      if (Array.isArray(resp.resources)) {
        resources = resources.concat(resp.resources.map(r => ({
    ...
    res.json({ ok: true, prefix, count: resources.length, items: resources.slice(0, 20) });
  } catch (e) {
    res.status(500).json({ ok: false, error: e?.message || String(e) });
  }
}); // end /api/cloudinary/list (dev-only)
...
app.get("/api/config", (_req, res) => {
  try {
    const safe = sanitizeConfigForClient(CONFIG);
    res.json(safe);
  } catch (e) {
...
app.post("/api/config", (req, res) => {
  const b = req.body || {};
  // --- normalize alternative client payload shapes for Cloudinary ---
  if (!b.cloudinary || typeof b.cloudinary !== "object") {
    const cn = b.cloudName ?? b.cloudinaryCloudName ?? b.cloudinary_name;
    const ak = b.apiKey ?? b.cloudApiKey ?? b.cloudinary_api_key;
    const as = b.apiSecret ?? b.cloudApiSecret ?? b.cloudinary_api_secret;
    const fd = b.folder ?? b.cloudFolder ?? b.cloudinary_folder;
    if (cn || ak || as || fd) {
      b.cloudinary = {
        cloudName: String(cn ?? ""),
        apiKey: String(ak ?? ""),
        apiSecret: typeof as === "string" ? as : "",
        folder: String(fd ?? "newzlettr"),
      };
    }
  }
  // Accept `imageHost` from either top-level or nested under `b.cloudinaryMode`
  if (typeof b.cloudinaryMode === "string" && typeof b.imageHost !== "string") {
    b.imageHost = b.cloudinaryMode; // e.g., "embedded" | "cloudinary"
  }
  ...
  if (b.cloudinary && typeof b.cloudinary === "object") {
    CONFIG.cloudinary = CONFIG.cloudinary || { cloudName: "", apiKey: "", apiSecret: "", folder: "newzlettr" };
    if (typeof b.cloudinary.cloudName === "string") CONFIG.cloudinary.cloudName = b.cloudinary.cloudName;
    if (typeof b.cloudinary.apiKey === "string") CONFIG.cloudinary.apiKey = b.cloudinary.apiKey;
    // only update apiSecret if a non-empty value is provided (so we don't wipe it on save)
    if (typeof b.cloudinary.apiSecret === "string" && b.cloudinary.apiSecret.trim().length > 0) {
      CONFIG.cloudinary.apiSecret = b.cloudinary.apiSecret;
    }
  ...
  saveConfig(CONFIG);
  // --- respond with sanitized config for UI ---
  res.json({ ok: true, config: sanitizeConfigForClient(CONFIG) });
});

app.get("/api/status", (_req, res) => {
  const s = CONFIG.lastTest || {};
  const imageHost = String(CONFIG.imageHost || "embedded").toLowerCase();
  const cloudinaryOk = imageHost === "embedded" ? true : s.cloudinary === "ok";
...
app.post("/api/cloudinary/test", async (req, res) => {
  try {
    // Allow testing either the current CONFIG or ad-hoc values from the request body
    const body = req.body || {};

    // If user selected Embedded in the UI, consider it OK without external call
    const mode = String(body.imageHost || CONFIG.imageHost || "embedded").toLowerCase();
    if (mode === "embedded") {
      // Persist mode if it changed
      if (CONFIG.imageHost !== "embedded") {
        CONFIG.imageHost = "embedded";
        saveConfig(CONFIG);
      }
      return res.json({ ok: true, mode: "embedded" });
    }

    // Expect Cloudinary creds either in body.cloudinary or CONFIG.cloudinary
    const incoming = body.cloudinary && typeof body.cloudinary === "object" ? body.cloudinary : {};
    const cloudName = String(incoming.cloudName ?? CONFIG?.cloudinary?.cloudName ?? "").trim();
    const apiKey    = String(incoming.apiKey    ?? CONFIG?.cloudinary?.apiKey    ?? "").trim();
    const apiSecret = String(incoming.apiSecret ?? CONFIG?.cloudinary?.apiSecret ?? "").trim();
    const folder    = String(incoming.folder    ?? CONFIG?.cloudinary?.folder    ?? "newzlettr").trim();

    if (!cloudName || !apiKey || !apiSecret) {
      return res.status(400).json({ ok: false, error: "Missing Cloudinary cloudName, apiKey, or apiSecret" });
    }

    // Call Cloudinary Admin API ping endpoint to validate credentials
    const base = `https://api.cloudinary.com/v1_1/${encodeURIComponent(cloudName)}/ping`;
    ...
    let r, json;
    try {
      r = await fetch(base, { headers: { Authorization: `Basic ${auth}` }, signal: ac.signal });
    ...
    if (!r || !r.ok) {
      const detail = json?.error?.message || `${r?.status || ""} ${r?.statusText || ""}`.trim();
      return res.status(200).json({ ok: false, error: detail || "Cloudinary ping failed" });
    }

    const status = (json && (json.status || json.ok)) || "ok";

    // Persist imageHost + cloudinary settings on success so UI can reload and see them
    CONFIG.imageHost = "cloudinary";
    CONFIG.cloudinary = {
      cloudName,
      apiKey,
      // Only store secret if the caller provided one (so we don't accidentally clear it)
      apiSecret: apiSecret || CONFIG.cloudinary?.apiSecret || "",
      folder: folder || "newzlettr",
    };
    // Optional: track lastTest for UI badges if needed later
    CONFIG.lastTest = { ...(CONFIG.lastTest || {}), cloudinary: status === "ok" ? "ok" : "fail" };
    saveConfig(CONFIG);

    return res.json({ ok: true, status: status === "ok" ? "ok" : String(status), mode: "cloudinary" });
  } catch (e) {
...
app.post("/api/test-email", async (req, res) => {
  ...
  try {
    ...
    if (hasHtml || hasText) {
      ...
      const subject = String(req.body.subject || "Newsletter").trim();
      ...
      const toHeader = toList.length ? toList.join(", ") : merged.fromAddress;
        ...
        let htmlBody = hasHtml ? req.body.html : undefined;
        let attachments;
        if (htmlBody) {
          const processed = await processImagesForEmail(
            htmlBody,
            { mode: String(CONFIG.imageHost || "embedded").toLowerCase(), cfg: CONFIG }
          );
        ...
        const msg = {
          ...
          headers: { "X-Newzlettr-Path": "/api/test-email" },
        };
...
async function getHomeSummary(days) {
  try {
    const url = apiUrl(`/api/tautulli/summary?days=${encodeURIComponent(days)}`);
    const r = await fetch(url, { headers: { 'x-internal-render': '1' } });
    if (!r.ok) return null;
    return await r.json();
  } catch { return null; }
}
...
async function buildOwnerRecommendationHtml() {
  ...
  try {
    const r = await fetch(apiUrl(`/api/plex/item/${encodeURIComponent(String(id))}`));
    if (r.ok) {
      const j = await r.json();
...
async function renderTemplate(html, historyDays) {
  ...
  // Prefer raw Tautulli home_stats so "Most Watched" matches the dashboard exactly
  let homeStats = [];
  try {
    const r = await fetch(
      apiUrl(`/api/tautulli/passthrough?cmd=get_home_stats&time_range=${encodeURIComponent(days)}`),
      { headers: { 'x-internal-render': '1' } }
    );
    if (r.ok) {
      const j = await r.json().catch(() => null);
      if (Array.isArray(j)) homeStats = j;
    }
  } catch {}
  ...
  const blocks = Array.isArray(homeStats) && homeStats.length
    ? homeStats
    : (Array.isArray(homeSummary?.home) ? homeSummary.home : []);
    ...
    await Promise.all(
      baseItems.map(async (it) => {
        if ((!it.href || !it.summary) && it.id != null) {
          try {
            const rr = await fetch(apiUrl(`/api/plex/item/${encodeURIComponent(String(it.id))}`));
            if (rr.ok) {
...
// If an <img src> points at our local /assets/* path, try to replace it with a known Cloudinary URL from the map
function mapLocalAssetToCloudinary(src) {
  try {
    const localPrefix = "/assets/";
    const absolutePrefix = absUrl(localPrefix);
    let rel = null;

    if (src.startsWith(absolutePrefix)) {
      rel = src.slice(absolutePrefix.length);
    } else if (src.startsWith(localPrefix)) {
      rel = src.slice(localPrefix.length);
    }

    if (!rel) return null;

    // Normalize Windows-style slashes just in case
    rel = rel.replace(/\\/g, "/");

    // The uploader stores keys relative to /public (e.g., "platforms/android.png")
    const mapped = CLOUDINARY_MAP[rel];
...
function extractImgSrcs(html) {
  ...
  while ((m = re.exec(html))) {
    ...
    if (!out.has(src)) out.set(src, { src, idx: out.size });
  }
  return [...out.values()];
}

async function fetchAsBuffer(url) {
  // Resolve relative URLs (e.g., "/api/plex/image?...") against our server base
  try {
  ...
  try {
    const r = await fetch(url, { headers: { "x-internal-render": "1" }, signal: ac.signal });
...
async function cloudinaryUpload({ buffer, contentType, filename, cfg }) {
  const cloudName = String(cfg?.cloudinary?.cloudName || "").trim();
  const apiKey    = String(cfg?.cloudinary?.apiKey || "").trim();
  const apiSecret = String(cfg?.cloudinary?.apiSecret || "").trim();
  const folder    = String(cfg?.cloudinary?.folder || "newzlettr").trim();
  if (!cloudName || !apiKey || !apiSecret) throw new Error("Cloudinary not configured");

  // Signed upload (sha1 of sorted params + api_secret)
  const timestamp = Math.floor(Date.now() / 1000);
  const paramsToSign = `folder=${encodeURIComponent(folder)}&overwrite=true&timestamp=${timestamp}`;
  ...
  try {
    ...
    const json = await r.json().catch(() => null);
    if (!r.ok || !json || !json.secure_url) {
      const msg = (json && (json.error?.message || json.message)) || `HTTP ${r.status}`;
      throw new Error(`Cloudinary upload failed: ${msg}`);
    }
    return { url: json.secure_url, bytes: json.bytes || buffer.length, width: json.width, height: json.height, format: json.format };
  } finally {
    clearTimeout(t);
  }
}

async function processImagesForEmail(html, { mode = "embedded", cfg }) {
  let outHtml = String(html || "");
  const images = extractImgSrcs(outHtml);
  if (images.length === 0) return { html: outHtml, attachments: [] };

  if (mode === "embedded") {
    const attachments = [];
    for (const it of images) {
      try {
        const { buffer, contentType } = await fetchAsBuffer(it.src);
        const cid = crypto.createHash("md5").update(buffer).digest("hex") + "@newzletr";
        const ext = (contentType.split("/")[1] || "bin").split(";")[0];
        const filename = `image.${ext}`;
        attachments.push({ filename, content: buffer, contentType, cid, contentDisposition: "inline" });
        outHtml = outHtml.replaceAll(it.src, `cid:${cid}`);
      } catch {
  ...
  if (mode === "cloudinary") {
    for (const it of images) {
      try {
        // Fast path: if this is a local /assets/* URL and we have a mapped Cloudinary URL, use it without uploading
        const mapped = mapLocalAssetToCloudinary(it.src);
        if (mapped) {
          outHtml = outHtml.replaceAll(it.src, mapped);
          continue;
        }

        // Otherwise, upload on-the-fly (e.g., dynamic Plex posters proxied via /api/plex/image)
        const { buffer, contentType } = await fetchAsBuffer(it.src);
        const ext = (contentType.split("/")[1] || "bin").split(";")[0];
        const filename = `image.${ext}`;
        const up = await cloudinaryUpload({ buffer, contentType, filename, cfg });
        outHtml = outHtml.replaceAll(it.src, up.url);
      } catch {
        // leave original src on failure
      }
    }
...
async function sendNewsletter(nl, { manual = false } = {}) {
    ...
    // Build wrapper HTML then process images per Settings (embedded | cloudinary)
    const mode = String(CONFIG.imageHost || "embedded").toLowerCase();
    const processed = await processImagesForEmail(
      `
...
app.post("/api/newsletters/:id/send-now", async (req, res) => {
  try {
    ...
    // Process images per current CONFIG (embedded -> inline attachments, cloudinary -> external URLs)
    if (html) {
      try {
        const mode = String(CONFIG.imageHost || "embedded").toLowerCase();
        const processed = await processImagesForEmail(html, { mode, cfg: CONFIG });
        html = processed.html;
        // Attach inline images only when in embedded mode and we have attachments
        var _imgAttachments = processed.attachments && processed.attachments.length ? processed.attachments : undefined;
      } catch (e) {
        console.error("[send-now] image processing failed:", e?.message || e);
      }
    }
...
// --- Debug: preview processed HTML (no send) ---
app.get("/api/newsletters/:id/preview", async (req, res) => {
  try {
    const id = String(req.params.id || "");
    const newsletters = loadNewslettersSafe();
    const nl = newsletters.find((x) => x.id === id);
    ...
    const before = extractImgSrcs(wrapper).map(x => x.src);
    const processed = await processImagesForEmail(wrapper, { mode: String(CONFIG.imageHost || "embedded").toLowerCase(), cfg: CONFIG });
    const after = extractImgSrcs(processed.html).map(x => x.src);
    ...
    res.json({
      ...
      srcsBefore: before,
      srcsAfter: after,
      htmlSample: processed.html.slice(0, 1000)
    });
  } catch (e) {
    res.status(500).json({ ok: false, error: e?.message || String(e) });
  }
